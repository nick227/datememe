...
export async function listPlans(_request: AuthenticatedRequest, reply: any) {
  const data = await subscriptionService.listPlans()
  return reply.send({ data })
}

export async function getMySubscription(request: AuthenticatedRequest, reply: any) {
  const data = await subscriptionService.getMySubscription(request.user.id)
  return reply.send({ data })
}

export async function devPurchaseSubscription(request: AuthenticatedRequest, reply: any) {
