...
Before major data/storage changes, take a database dump and a volume snapshot or file archive, retain them off-service, and verify they can be read. Railway's snapshot API returned `Not Authorized` for this session; the staging database instead has private off-volume logical backups on the operator machine. A readable archive of all six staging media files was also saved after seeding. Backups are retained under `~/.local/state/datememe/backups/2026-09-25/` with private permissions. An on-volume backup alone is insufficient. Arrange recurring retained database and media backups before production use. A restore must pair the database metadata with the corresponding files.
