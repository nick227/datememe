...
The first schema setup used a guarded, one-time `prisma db push` against the new staging database, followed by the existing taxonomy seed. It is not a pre-deploy command or an automated migration strategy. Introduce versioned migrations in milestone 2 before automated hosted deployments.
...
The installed app version comes from `expo.version` in `app.json`, currently `1.0.0`; the workspace package version is not a store version. QA currently uses Android version code `1`. Monotonic release numbering is introduced with Play in milestone 3.
...
An upload is not deployment success: confirm that returned deployment ID reaches `SUCCESS` and verify the hosted API. No source auto-deployment or pre-deploy schema command was configured for these new services.

Media is still temporary local storage. Email and push delivery remain simulated; native purchases are not implemented and the server disables the simulated purchase endpoint in production runtime mode. These limitations are expected for this first private APK. Do not use real personal data or treat this environment as a public launch.
